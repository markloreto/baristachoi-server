...
class ApiController extends BaseController
{
    ...
    public function getTopLocations(){
        $region = DB::table("machines AS m")->select(DB::raw('IFNULL(m.region, "Unknown Region") AS `region`, COUNT(*) AS `total`, (SELECT name FROM depots WHERE id = m.depot_id) AS `depot_name`'))->orderBy(\DB::raw('count(*)'), 'DESC')->groupBy('m.region')->limit(10)->get();
