...
class ApiController extends BaseController
{
    ...
    public function clientFilter(Request $request){
        ...
        if($clientId){
            ...
            if($contact){
                $filter->where(function ($query) use($contact) {
                    $query->whereRaw("(SELECT cnt.contact FROM contacts cnt WHERE cnt.reference_id = c.id) LIKE '%".$contact."%'");
                });
            }
    ...
    public function dealerVersion(){
        ...
        $arr["version"] = 4.2;
        $arr["changelog"] = array(
            array("ADDED", "Verfied Column for machine")  
        );
