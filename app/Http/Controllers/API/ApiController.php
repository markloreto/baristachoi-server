...
class ApiController extends BaseController
{
    ...
    public function getTopLocations(){
        $region = DB::table("machines")->select(DB::raw('IFNULL(region, "Unknown Region") AS `region`, COUNT(*) AS `total`, (SELECT name FROM depots WHERE depot_id = machine.depot_id) AS `depot_name`'))->orderBy(\DB::raw('count(*)'), 'DESC')->groupBy('region')->limit(10)->get();
