    ...
    public function productivityView(Request $request){
        ...
        if($type == "dayGridMonth"){
            ...
            foreach($records AS $record){
                ...
                ->whereRaw("m.delivery = DATE_FORMAT('".$record->csDate."', '%a') AND m.created_at <= DATE('".$end."') AND m.staff_id = '".$dealerId."' AND m.id IN (SELECT id FROM callsheets WHERE machine_id = m.id AND created_at >= '".$f->toDateTimeString()."' AND created_at <= '".$end."')")
                ->count();
