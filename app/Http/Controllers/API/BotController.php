...
class BotController extends BaseController
{
    ...
    public function getBotProductCategoryList(Request $request){
      ...
      ->select(DB::raw('ppc.*, (SELECT IF(pp.virtual_cost, 1, (SELECT COUNT(id) FROM pabile_inventories WHERE product_id = pp.id AND order_id IS NULL)) FROM pabile_products pp WHERE pp.category_id = '.$catId.') AS prodCount'))
      ->having("prodCount", "!=", 0);
    ...
    public function botSearchProduct(Request $request){
      ...
      if($totalRecords){
        foreach($records as $r){
          $thumb = 'https://markloreto.xyz/botPhotoGallery/' . $r->id . "?t=" . $r->updated_date;
          $items[] = [
            "title" => "[₱ " . $r->price . "] " . $r->name . (($r->brand) ? ", " . $r->brand : "") . (($r->weight) ? ", " . $r->weight : "") . (($r->color) ? ", " . $r->color : "") . (($r->flavor) ? ", " . $r->flavor : "") . (($r->size) ? ", " . $r->size : "") . (($r->size) ? ", " . $r->size : "") . (($r->manufacturer) ? ", " . $r->manufacturer : "") . (($r->dimension) ? ", " . $r->dimension : "") . (($r->type) ? ", " . $r->type : "") . (($r->unit) ? ", " . $r->unit : ""),
            "subtitle" => $r->description,
            "image_url" => $thumb,
            "buttons" => [
                [
