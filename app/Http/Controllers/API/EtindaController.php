...
class EtindaController extends BaseController
{
    ...
    public function searchProducts(Request $request){
        ...
        if($returnAsData){
            ...
            $records = $records->addSelect(DB::raw("(SELECT COUNT(*) FROM pabile_inventories WHERE order_id = ".$orderId." AND product_id = pp.id) AS qty"))->WhereIn("pp.id", $ids)->get();
    ...
    public function getProducts(Request $request){
        ...
        $records = DB::table("pabile_products as pp")
        ->select(DB::raw('pp.*, (SELECT COUNT(id) FROM pabile_inventories pi WHERE pi.product_id = pp.id AND pi.order_id IS NULL) AS inventory, (SELECT value FROM pabile_product_specs WHERE `key` = 6 AND product_id = pp.id) AS brand, (SELECT value FROM pabile_product_specs WHERE `key` = 1 AND product_id = pp.id) AS weight, (SELECT value FROM pabile_product_specs WHERE `key` = 2 AND product_id = pp.id) AS `color`, (SELECT thumbnail FROM pabile_product_photos WHERE product_id = pp.id AND `primary` = 1) AS `thumbnail`, (SELECT COUNT(*) FROM pabile_product_photos WHERE product_id = pp.id) AS `numPhotos`, (SELECT COUNT(*) FROM pabile_product_tags WHERE product_id = pp.id) AS `numTags`'));
        if($categoryId > 0){
            $records = $records->where("pp.category_id", $categoryId)->get();
        }else{
            if($categoryId === 0)
                $records = $records->having('numPhotos', '=', 0)->get();
            else
                $records = $records->having('numTags', '=', 0)->get();
        }
        //->having('mySold', '!=', 0)
    ...
    public function deliveredOrder(Request $request){
        ...
        return $this->sendResponse($order, 'deliveredOrder');
    }
    ...
    public function updateMobilePrefix(){
        ...
        foreach($records as $record){
            $uMobilePrefix = intval(substr($record->mobile, 0, 4));

            $prefixRec = DB::table("pabile_mobile_prefixes")->where("prefix", $uMobilePrefix)->first();
            
            if($prefixRec == null){
                $uMobilePrefix = intval(substr($record->mobile, 0, 3));
    ...
    public function remitVirtual(Request $request){
        ...
        return $this->sendResponse($ids, 'remitVirtual');
    }
    
    public function getVirtualList(Request $request){
        $data = $request->all();
        $offset = $data["offset"];
