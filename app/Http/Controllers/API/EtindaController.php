...
class EtindaController extends BaseController
{
    ...
    public function getPurchases(Request $request){
        ...
        $records = DB::table("pabile_inventories as pi")->select(DB::raw('pi.purchase_id, (SELECT created_at FROM pabile_purchases WHERE id = pi.purchase_id) AS `Date`, (SELECT name FROM pabile_products WHERE id = pi.product_id) AS `Product Name`, pi.cost AS `Cost`, (SELECT COUNT(id) FROM pabile_inventories WHERE purchase_id = pi.purchase_id) AS `Loaded`, COUNT(pi.id) AS `Remaining`'))
