...
class EtindaController extends BaseController
{
    ...
    public function getOrderItems(Request $request){
        ...
        ->select(DB::raw('pp.*, ' . ($returnAsData) ? 'pi.qty' : 'COUNT(pi.product_id) AS `qty`' . ', (SELECT photo FROM pabile_product_photos WHERE product_id = pp.id AND `primary` = 1) AS `Photo`, (SELECT name FROM pabile_product_categories WHERE id = pp.category_id) AS categoryName'))
